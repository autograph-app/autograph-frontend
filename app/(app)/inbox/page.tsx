...
export default function ArtistInboxPage() {
  ...
  useEffect(() => {
    if (user && user.accountType !== 1) {
      toast.error('Only Artist accounts can access the Inbox.');
      router.push('/feed');
      return;
    }
    fetchRequests();
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      if (response.data?.success) {
        toast.success('Artwork signed and watermarked successfully!');
        fetchRequests();
      } else {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || 'Error approving signature.');
    } finally {
  ...
  if (loading && requests.length === 0) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center text-zinc-400">
        ...
        <p className="text-sm">Loading signature requests...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-zinc-400 text-sm">Review, sign, and certify autograph requests from your fans</p>
      </div>
          ...
          {pendingRequests.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center p-12 bg-zinc-950/40 rounded-3xl border border-white/5 space-y-4">
              ...
              <div>
                ...
                <p className="text-zinc-500 text-xs mt-1">No pending signature requests at the moment.</p>
              </div>
            </div>
          ) : (
                    ...
                    <div className="space-y-2 min-w-0">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge className="bg-violet-500/10 text-violet-400 border border-violet-500/20 text-[10px]">Fan</Badge>
                        </div>
                        <span className="text-[10px] text-zinc-500 flex items-center gap-1 mt-0.5">
                          <Clock className="h-3 w-3" /> Requested on {new Date(req.createdDate).toLocaleDateString()}
                        </span>
                      </div>
        ...
        <TabsContent value="history" className="space-y-4">
          {resolvedRequests.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center p-12 bg-zinc-950/40 rounded-3xl border border-white/5 space-y-4">
              ...
              <div>
                <h3 className="font-bold text-white text-sm">No history yet</h3>
                <p className="text-zinc-500 text-xs mt-1">Autographs you approve or reject will appear here.</p>
              </div>
            </div>
          ) : (
